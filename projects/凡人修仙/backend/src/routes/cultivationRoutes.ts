import { Router } from 'express';
import {
  startCultivationHandler,
  endCultivationHandler,
  getCultivationHistoryHandler,
  getCultivationStatusHandler
} from '../controllers/cultivationController';
import { authenticate } from '../middleware/auth';

const router = Router();

/**
 * @route   POST /api/v1/cultivate/start
 * @desc    开始修炼
 * @access  Private
 * @body    { city?: string }
 * @response { success: boolean, data: { cultivation: {...}, tianShi: {...} } }
 */
router.post('/start', authenticate, startCultivationHandler);

/**
 * @route   POST /api/v1/cultivate/end
 * @desc    结束修炼
 * @access  Private
 * @response { success: boolean, data: { result: {...} } }
 */
router.post('/end', authenticate, endCultivationHandler);

/**
 * @route   GET /api/v1/cultivate/history
 * @desc    获取修炼历史
 * @access  Private
 * @query   { page?: number, pageSize?: number }
 * @response { success: boolean, data: { logs: [...], pagination: {...} } }
 */
router.get('/history', authenticate, getCultivationHistoryHandler);

/**
 * @route   GET /api/v1/cultivate/status
 * @desc    获取当前修炼状态
 * @access  Private
 * @response { success: boolean, data: { cultivation: {...}, bazi: {...} } }
 */
router.get('/status', authenticate, getCultivationStatusHandler);

export default router;